...
import {
  ...
  signOut, sendPasswordResetEmail, type Auth, type User as FirebaseUser,
} from "firebase/auth";
...
function loadLastTab(): "home" | "today" | "protocol" | "squad" | "score" {
  try {
    const t = localStorage.getItem(TAB_KEY);
    if (t === "home" || t === "today" || t === "protocol" || t === "squad" || t === "score") return t;
  } catch {}
  return "home";
}
...
const ELITE_FEATURES = [
  ...
  "Recovery Score — a live 1–100 score (red/yellow/green, just like a credit score) built from your real task completion and how well your calories match your target",
...
function computeRecoveryScore(saved: SavedState, macros: { calories: number } | null): { score: number; taskScore: number; calorieScore: number | null } {
  ...
  const score = calorieScore != null ? Math.round(taskScore * 0.6 + calorieScore * 0.4) : taskScore;
  return { score: Math.max(1, Math.min(100, score)), taskScore, calorieScore };
}
...
// way a credit score climbs with good habits.
  ...
  return (
    <div style={{ width: w, textAlign: "center" }}>
      <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`}>
        ...
        <text x={cx} y={cy - r * 0.42} fontSize={w * 0.17} fontWeight={900} fill={band.color} textAnchor="middle">{clamped}</text>
        <text x={cx} y={cy - r * 0.42 + w * 0.065} fontSize={Math.max(9, w * 0.042)} fontWeight={800} fill={band.color} textAnchor="middle" style={{ letterSpacing: "1.5px" }}>{band.label}</text>
      </svg>
    </div>
  );
}
  ...
  return (
    <div style={{ textAlign: "center", padding: "24px 4px" }}>
      ...
      <p style={{ fontSize: 13, color: T.textSecondary, marginBottom: 16, lineHeight: 1.6 }}>
        A live 1–100 score — red, yellow, green, just like a credit score — built from your real task completion and how well your calories track your target. Plus a full daily food and exercise log to drive it.
      </p>
      ...
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 430, background: T.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: "20px 20px calc(24px + env(safe-area-inset-bottom,0px))", maxHeight: "85vh", overflowY: "auto", boxSizing: "border-box" }}>
        ...
        {!cloudSyncConfigured ? (
          ...
          <div>
            ...
            <button onClick={handleSubmit} disabled={busy || !email.trim() || (mode !== "reset" && !password)}
              ...
              <LogIn size={15} /> {busy ? "Please wait…" : mode === "reset" ? "Send reset email" : mode === "signup" ? "Create account" : "Sign in"}
            </button>
            {mode !== "reset" ? (
...
function AppDashboard({ saved, onReset, onUpgrade, onOpenAccount, authUser }: {
  ...
  const [tab, setTabRaw] = useState<"home" | "today" | "protocol" | "squad" | "score">(() => loadLastTab());
  const setTab = useCallback((t: "home" | "today" | "protocol" | "squad" | "score") => {
          ...
          {!eliteActive ? (
            ...
            <>
              ...
              <p style={{ fontSize:13, color:T.textSecondary, marginBottom:16 }}>
                Live, just like a credit score — built from your real task completion and how well your calories track your target.
              </p>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                <Card style={{ textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 10, color: T.textMuted, fontWeight: 700, marginTop: 2 }}>TASK COMPLETION</div>
                </Card>
                <Card style={{ textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 10, color: T.textMuted, fontWeight: 700, marginTop: 2 }}>CALORIE MATCH</div>
                </Card>
              </div>
              {recoveryScore.calorieScore == null && (
                <div style={{ fontSize: 11, color: T.textMuted, marginBottom: 16, textAlign: "center" }}>Log a few meals below (and set up your Body Profile in Protocol) to activate the calorie-match half of your score.</div>
              )}
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: T.bg }}>
      ...
      <div style={{ flexShrink: 0, background: T.surface, borderTop: `1px solid ${T.border}`, paddingBottom: "env(safe-area-inset-bottom,12px)", paddingTop: 10, paddingLeft: 4, paddingRight: 4, display: "flex", justifyContent: "space-around", alignItems: "flex-end", zIndex: 50 }}>
        ...
        <NavBtn icon={<Gauge size={20} />}   label="Score"   active={tab === "score"}   onClick={() => setTab("score")} />
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const finalizeSetup = (plan: "pro" | "elite" | "trial", code: string) => {
    ...
    const ns: SavedState = {
      ...
      completedTasks: {}, suppChecked: {}, bodyProfile: null, dailyLogs: {}, posts: {},
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, height: "100dvh", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily: "'Inter',sans-serif", background: T.bg, overscrollBehavior: "none" }}>
      <GlobalMobileStyles />
