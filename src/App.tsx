...
import { Check, ChevronRight, Trophy, Users, Home, BookOpen, Dumbbell, RotateCcw, Heart, MessageCircle, Send, ArrowLeft } from "lucide-react";

// ============================================================================
// COMEBACK — PRO (Full Unlocked Edition)
// Single standalone paid app. No ads, no plan picker, no paywalls — every
// feature is unlocked from the first session: personalised macros, the full
// supplement + nutrition system, squad, milestones, and the premium
// Animated Trophy/Badge Companion that levels up baby → bronze → silver →
// gold → diamond as the user's streak builds.
// ============================================================================
...
function GlobalBadgeStyles() {
  return (
    <style>{`
      @keyframes badgePulse { 0%,100% { opacity:0.16; transform:scale(1); } 50% { opacity:0.32; transform:scale(1.08); } }
      @keyframes badgeSpin { from { transform:rotate(0deg); } to { transform:rotate(360deg); } }
      @keyframes sparkleFloat { 0% { opacity:0; transform:translateY(0) scale(0.5); } 40% { opacity:1; transform:translateY(-8px) scale(1); } 100% { opacity:0; transform:translateY(-16px) scale(0.5); } }
    `}</style>
  );
}

// ── BADGE COMPANION DATA ──────────────────────────────────────────────────────
// Tiers: baby(day1-6) -> bronze(7-20) -> silver(21-59) -> gold(60-89) -> diamond(90+)
const BADGE_TIERS = [
  { id:"baby",    label:"Rookie",  min:1,  base:"#D5D8E3", edge:"#8E93A6", glow:"#AEB3C4", emblem:"star" as const },
  { id:"bronze",  label:"Bronze",  min:7,  base:"#E3A45B", edge:"#8B4A18", glow:"#CD7F32", emblem:"star" as const },
  { id:"silver",  label:"Silver",  min:21, base:"#EDEFF5", edge:"#9AA0B4", glow:"#D6DAE6", emblem:"star" as const },
  { id:"gold",    label:"Gold",    min:60, base:"#FFDE7A", edge:"#B8860B", glow:"#FFD700", emblem:"crown" as const },
  { id:"diamond", label:"Diamond", min:90, base:"#BFFFF3", edge:"#39FF14", glow:"#7DFFEA", emblem:"diamond" as const },
];
function getBadgeTier(dayNum: number) {
  let t = BADGE_TIERS[0];
  for (const b of BADGE_TIERS) { if (dayNum >= b.min) t = b; }
  return t;
}
function getNextBadgeTier(dayNum: number) {
  const cur = getBadgeTier(dayNum);
  const idx = BADGE_TIERS.indexOf(cur);
  return idx < BADGE_TIERS.length - 1 ? BADGE_TIERS[idx + 1] : null;
}
const BADGE_AURA_COLORS = ["#39FF14","#FF3B3B","#6366F1","#F59E0B","#EC4899","#38BDF8","#A855F7","#10B981"];
const BADGE_QUOTES: Record<string,string[]> = {
  streak:[
...
const STAR_PATH = "M0,-46 L13,-14 L46,-14 L19,7 L28,42 L0,20 L-28,42 L-19,7 L-46,-14 L-13,-14 Z";
const DIAMOND_PATH = "M0,-46 L32,-8 L0,50 L-32,-8 Z";
const DIAMOND_FACETS = ["M0,-46 L0,50", "M-32,-8 L32,-8", "M-16,-27 L0,-46 L16,-27", "M-16,-27 L-32,-8", "M16,-27 L32,-8"];
const CROWN_PATH = "M-22,8 L-22,-8 L-11,2 L0,-16 L11,2 L22,-8 L22,8 Z";
const SPARKLE_OFFSETS = [
  { x: -1.1, y: -0.9 }, { x: 1.15, y: -0.6 }, { x: -0.8, y: 1.0 }, { x: 1.0, y: 0.95 },
];

// ── BADGE COMPANION (SVG) ─────────────────────────────────────────────────────
function BadgeCompanion({ dayNum, mood = "happy", size = 90, auraColor = T.neon }: {
  dayNum: number; mood?: "happy"|"excited"|"neutral"|"sad"; size?: number; auraColor?: string;
}) {
  const tier = getBadgeTier(dayNum);
  const s = size;
  const cx = s * 0.5;
  const cy = s * 0.44;
  const outerR = s * 0.33;
  const gradId = `badgeGrad-${tier.id}-${size}`;
  const isSad = mood === "sad";
  const isNeutral = mood === "neutral";
  const showGlow = !isSad && !isNeutral;
  const showShine = (tier.id === "gold" || tier.id === "diamond") && !isSad && !isNeutral;
  const showSparkles = mood === "excited";
  const ribbonLeft = `${cx-outerR*0.55},${cy+outerR*0.55} ${cx-outerR*0.15},${cy+outerR*0.55} ${cx-outerR*0.05},${s*0.97} ${cx-outerR*0.65},${s*0.86}`;
  const ribbonRight = `${cx+outerR*0.55},${cy+outerR*0.55} ${cx+outerR*0.15},${cy+outerR*0.55} ${cx+outerR*0.05},${s*0.97} ${cx+outerR*0.65},${s*0.86}`;
  return (
    <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`} style={isSad ? { filter: "grayscale(0.55) brightness(0.8)" } : {}}>
      <defs>
        <radialGradient id={gradId} cx="35%" cy="30%" r="75%">
          <stop offset="0%" stopColor="#FFFFFF" stopOpacity="0.9" />
          <stop offset="35%" stopColor={tier.base} />
          <stop offset="100%" stopColor={tier.edge} />
        </radialGradient>
      </defs>
      <polygon points={ribbonLeft} fill={tier.edge} opacity={0.9} />
      <polygon points={ribbonRight} fill={tier.edge} opacity={0.9} />
      {showGlow && (
        <circle cx={cx} cy={cy} r={outerR*1.35} fill={auraColor} opacity={0.18}
          style={{ transformOrigin: `${cx}px ${cy}px`, animation: `badgePulse ${mood==="excited" ? "1.1s" : "2.6s"} ease-in-out infinite` }} />
      )}
      <circle cx={cx} cy={cy} r={outerR*1.12} fill="none" stroke={tier.glow} strokeWidth={s*0.015} opacity={0.5} />
      <circle cx={cx} cy={cy} r={outerR} fill={`url(#${gradId})`} stroke={tier.edge} strokeWidth={s*0.022} />
      <circle cx={cx} cy={cy} r={outerR*0.8} fill="none" stroke="white" strokeOpacity={0.3} strokeWidth={s*0.008} />
      {showShine && (
        <g style={{ transformOrigin: `${cx}px ${cy}px`, animation: "badgeSpin 5s linear infinite" }}>
          <ellipse cx={cx} cy={cy - outerR*0.5} rx={outerR*0.5} ry={outerR*0.18} fill="white" opacity={0.35} />
        </g>
      )}
      {tier.emblem === "crown" && (
        <path d={CROWN_PATH} transform={`translate(${cx},${cy - outerR*0.85}) scale(${outerR/60})`} fill={tier.glow} stroke={tier.edge} strokeWidth={2} />
      )}
      {tier.emblem === "diamond" ? (
        <g transform={`translate(${cx},${cy}) scale(${outerR/60})`}>
          <path d={DIAMOND_PATH} fill="white" opacity={0.92} />
          {DIAMOND_FACETS.map((d, i) => <path key={i} d={d} stroke={tier.edge} strokeWidth={2} opacity={0.4} />)}
        </g>
      ) : (
        <path d={STAR_PATH} transform={`translate(${cx},${cy}) scale(${outerR/60})`} fill="white" opacity={0.92} />
      )}
      {showSparkles && SPARKLE_OFFSETS.map((o, i) => (
        <circle key={i} cx={cx + o.x*outerR} cy={cy + o.y*outerR} r={s*0.022} fill={tier.glow}
          style={{ animation: `sparkleFloat 1.6s ease-in-out ${i*0.2}s infinite` }} />
      ))}
    </svg>
  );
}
...
function Welcome({ onNext }: { onNext: () => void }) {
  return (
    <div style={{ minHeight: "100%", display: "flex", flexDirection: "column", padding: "48px 24px 32px", background: `linear-gradient(160deg,${T.surface} 0%,${T.bg} 70%)` }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center" }}>
        ...
        <div style={{ marginTop: 32, marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 15, color: T.textSecondary, lineHeight: 1.6, maxWidth: 300, margin: "0 auto" }}>
            Science-backed recovery. Personalised nutrition. A trophy companion that grows with you.
          </p>
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center", marginBottom: 32 }}>
          {["🏆 Pro protocols", "📿 Daily faith", "🧪 Personalised macros", "🏅 Badge Companion", "📅 Real tracking"].map((f, i) => (
            <div key={i} style={{
      ...
      <p style={{ textAlign: "center", fontSize: 11, color: T.textMuted, marginTop: 12 }}>Everything unlocked · No ads · Progress saved automatically</p>
    </div>
  );
}
...
// ── BADGE SETUP (pick aura color + nickname) ─────────────────────────────────
function BadgeSetup({ onSave, onBack }: { onSave: (b: Badge) => void; onBack: () => void }) {
  const [auraColor, setAuraColor] = useState(BADGE_AURA_COLORS[0]);
  ...
  return (
    <div style={{ minHeight:"100%", background:T.bg, display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ padding:"20px 20px 0", position:"sticky", top:0, background:T.bg, zIndex:10 }}>
        ...
        <h2 style={{ fontWeight:900, fontSize:22, color:T.textPrimary, marginBottom:4 }}>Set Up Your Badge</h2>
        <p style={{ fontSize:13, color:T.textSecondary, marginBottom:12 }}>
          Starts as a <strong style={{color:T.neon}}>Rookie</strong> badge on day 1 and levels up to Bronze, Silver, Gold, then Diamond as your streak builds. 🏅
        </p>
      </div>
      <div style={{ flex:1, overflowY:"auto", padding:"0 20px 32px" }}>
        <div style={{ display:"flex", justifyContent:"center", marginBottom:20 }}>
          <div style={{ background:T.surface, borderRadius:24, padding:"20px 24px", border:`2px solid ${T.neon}50`, boxShadow:shadow.neon, display:"inline-flex", flexDirection:"column", alignItems:"center", gap:10 }}>
            <BadgeCompanion dayNum={1} mood={previewMood} size={140} auraColor={auraColor} />
            <div style={{ textAlign:"center" }}>
              <div style={{ fontWeight:900, fontSize:15, color:T.neon, textShadow:shadow.neon }}>
                {nickname || "Name your badge"} · Rookie
              </div>
        ...
        <div style={{ marginBottom:20 }}>
          <div style={{ fontWeight:800, fontSize:12, color:T.neon, marginBottom:8, letterSpacing:"1px", textTransform:"uppercase" }}>Name Your Badge</div>
          <input
            value={nickname}
        ...
        <div style={{ display:"flex", gap:10, flexWrap:"wrap", marginBottom:24 }}>
          {BADGE_AURA_COLORS.map(c => (
            <button key={c} onClick={() => setAuraColor(c)}
              style={{ width:42, height:42, borderRadius:21, background:c, cursor:"pointer", border:`3px solid ${auraColor===c?"white":"transparent"}`, boxShadow:auraColor===c?`0 0 14px ${c}`:undefined }}/>
          ))}
        </div>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end" }}>
            {BADGE_TIERS.map(tier => (
              <div key={tier.id} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
                <BadgeCompanion dayNum={tier.min} mood="happy" size={46} auraColor={auraColor} />
                <div style={{ fontSize:9, color:T.textMuted, fontWeight:700, textAlign:"center" }}>Day {tier.min}<br/>{tier.label}</div>
              </div>
            ))}
          </div>
        </div>
        <button onClick={() => onSave({ auraColor, nickname: nickname || "Comeback Badge" })}
          ...
          {nickname ? `Save ${nickname} ✨` : "Save My Badge ✨"}
        </button>
      </div>
    </div>
  );
...
function AppDashboard({ saved, onReset }: { saved: SavedState; onReset: () => void }) {
  const [tab, setTab] = useState<"home" | "today" | "protocol" | "squad" | "badge">("home");
  ...
  const badgeMood: "happy" | "sad" | "excited" | "neutral" =
    allDone ? "excited" : doneCount >= tasks.length * 0.6 ? "happy" : !saved.streakAlive ? "sad" : "neutral";
  const badgeQuote = allDone
    ? BADGE_QUOTES.goal[dayNum % BADGE_QUOTES.goal.length]
    : !saved.streakAlive
    ? BADGE_QUOTES.miss[dayNum % BADGE_QUOTES.miss.length]
    : doneCount === 0
    ? BADGE_QUOTES.morning[dayNum % BADGE_QUOTES.morning.length]
    : BADGE_QUOTES.streak[dayNum % BADGE_QUOTES.streak.length];
  const macros = bodyProfile ? calcMacros(bodyProfile.height, bodyProfile.weight, bodyProfile.gender, saved.goalId) : null;
  const tier = getBadgeTier(dayNum);
  const nextTier = getNextBadgeTier(dayNum);
          ...
          {saved.badge && (
            <NeonCard style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <BadgeCompanion dayNum={dayNum} mood={badgeMood} size={72} auraColor={saved.badge.auraColor} />
                <div style={{ flex: 1 }}>
                  <div style={{fontWeight:800,fontSize:10,color:T.neon,marginBottom:4,letterSpacing:"1px"}}>{saved.badge.nickname.toUpperCase()} SAYS</div>
                  <p style={{ fontSize: 13, color: T.textPrimary, lineHeight: 1.5, fontStyle: "italic", margin: 0 }}>{badgeQuote}</p>
                </div>
              </div>
            </NeonCard>
          )}
          ...
          <div style={{ padding: "0 18px" }}>
            ...
            {allDone && (
              <Card style={{ background: T.neonL, border: `1.5px solid ${T.neon}`, marginTop: 10, textAlign: "center", padding: "20px 16px", boxShadow: shadow.neon }}>
                ...
                {saved.badge && <div style={{ marginTop:12,display:"flex",justifyContent:"center" }}><BadgeCompanion dayNum={dayNum} mood="excited" size={80} auraColor={saved.badge.auraColor} /></div>}
              </Card>
            )}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", background: T.bg, padding: "20px 18px 100px" }}>
          {!saved.badge ? (
            <div style={{ textAlign: "center", padding: "40px 20px" }}>
              ...
              <h2 style={{fontWeight:900,fontSize:22,color:T.textPrimary,marginBottom:8}}>Your Badge Companion</h2>
              <p style={{fontSize:14,color:T.textSecondary,marginBottom:16,lineHeight:1.6}}>Finish setup from onboarding to unlock your Badge Companion.</p>
            </div>
          ) : (
            <>
              <h1 style={{ fontWeight:900, fontSize:22, color:T.textPrimary, marginBottom:4 }}>{saved.badge.nickname} 🏅</h1>
              <p style={{ fontSize:13, color:T.textSecondary, marginBottom:20 }}>
                Your badge levels up every day you stay consistent. Currently <strong style={{color:T.neon}}>{tier.label}</strong>{nextTier ? ` · ${nextTier.min - dayNum > 0 ? nextTier.min - dayNum : 0} days to ${nextTier.label}` : " · Max tier reached"}.
              </p>
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
                <div style={{ background: T.surface, borderRadius: 24, padding: 24, border: `2px solid ${T.neon}50`, boxShadow: shadow.neon, display: "inline-flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                  <BadgeCompanion dayNum={dayNum} mood={badgeMood} size={150} auraColor={saved.badge.auraColor} />
                  <div style={{ textAlign: "center" }}>
                    <div style={{ fontWeight:800, fontSize:14, color:T.neon, textShadow:shadow.neon }}>{saved.badge.nickname}</div>
                    <div style={{ fontSize:11, color:T.textMuted }}>{tier.label} Badge · Day {dayNum}</div>
                  </div>
                </div>
              </div>
              <NeonCard style={{ marginBottom: 16 }}>
                <div style={{fontWeight:800,fontSize:10,color:T.neon,marginBottom:8,letterSpacing:"1px"}}>{saved.badge.nickname.toUpperCase()} SAYS</div>
                <p style={{ fontSize: 14, fontStyle: "italic", color: T.textPrimary, lineHeight: 1.65, fontFamily: "Georgia,serif", margin: 0 }}>{badgeQuote}</p>
              </NeonCard>
              <Card style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  {BADGE_TIERS.map(t => (
                    <div key={t.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, opacity: dayNum >= t.min ? 1 : 0.4 }}>
                      <BadgeCompanion dayNum={t.min} mood="happy" size={44} auraColor={saved.badge!.auraColor} />
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: T.bg }}>
      ...
      <div style={{ flexShrink: 0, background: T.surface, borderTop: `1px solid ${T.border}`, paddingBottom: "env(safe-area-inset-bottom,12px)", paddingTop: 10, paddingLeft: 4, paddingRight: 4, display: "flex", justifyContent: "space-around", alignItems: "flex-end", zIndex: 50 }}>
        ...
        <NavBtn icon={<Trophy size={20} />}  label="Badge"   active={tab === "badge"}   onClick={() => setTab("badge")} />
      </div>
    </div>
  );
}
...
export default function App() {
  type Screen = "welcome" | "sport" | "injury" | "severity" | "goal" | "badge" | "dashboard";
  ...
  const handleBadgeSave = (badge: Badge) => {
    ...
    const ns: SavedState = {
      ...
      completedTasks: {}, suppChecked: {}, bodyProfile: null, badge, posts: {},
    };
  ...
  return (
    <div style={{ width: "100%", height: "100dvh", maxWidth: 430, margin: "0 auto", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily: "'Inter',sans-serif", background: T.bg }}>
      ...
      {screen === "goal"     && <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column" }}><GoalSelect     onNext={g  => { setGoal(g);     setScreen("badge");    }} /></div>}
      {screen === "badge"    && <BadgeSetup onSave={handleBadgeSave} onBack={() => setScreen("goal")} />}
