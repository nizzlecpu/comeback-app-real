...
import { Check, ChevronRight, Trophy, Users, Home, BookOpen, Dumbbell, RotateCcw, Heart, MessageCircle, Send, ArrowLeft, Lock, Crown, X, Clock, LogIn, LogOut, Cloud, User as UserIcon } from "lucide-react";
...
function loadLastTab(): "home" | "today" | "protocol" | "squad" | "card" {
  try {
    const t = localStorage.getItem(TAB_KEY);
    if (t === "home" || t === "today" || t === "protocol" || t === "squad" || t === "card") return t;
  } catch {}
  return "home";
}
...
// ── STATS CARD — FIFA-style Trading Card (SVG + CSS, Elite-exclusive) ─────────
// Every number on this card maps to something the person actually did:
// CON = % of logged days they completed every task, MOM = current streak
// strength, REC = supplement/recovery-protocol adherence, PRG = % through
// their full recovery program. OVR is just the average of those four — so a
// 91 OVR card is genuinely earned, not decorative. The old version drew the
// "athlete" as three rectangles and a circle, which read as a mannequin —
// this version drops the body entirely in favor of a glowing sport emblem
// (crest + energy rings + motion lines), closer to how real trading cards
// lean on typography, ratings and a bold central icon rather than a literal
// figure. The neon-green under-glow on every card (independent of tier or
// the user's chosen accent color) is the one constant tying every card back
// to the app's own brand color, no matter how it's customized.
function StatsCard({
  dayNum, streak, mood = "happy", size = 150, accentColor = T.neon, sportIcon = "🏆",
  consistencyPct, recoveryPct, progressPct,
}: {
  dayNum: number; streak: number; mood?: "happy"|"excited"|"neutral"|"sad"; size?: number;
  accentColor?: string; sportIcon?: string;
  consistencyPct?: number; recoveryPct?: number; progressPct?: number;
}) {
  ...
  const compact = size < 100; // thumbnails (tier rows, milestones) skip OVR/stat detail — no room to render it legibly

  const momentum    = Math.max(20, Math.min(99, Math.round(streak * 3 + 20)));
  const consistency = consistencyPct ?? Math.max(30, Math.min(99, Math.round((streak / Math.max(1, dayNum)) * 100)));
  const recovery     = recoveryPct ?? 75;
  const progress      = progressPct ?? Math.max(5, Math.min(99, Math.round((dayNum / 90) * 100)));
  const ovr = Math.round((momentum + consistency + recovery + progress) / 4);
  ...
  return (
    <div style={{
      ...
      boxShadow: `0 0 ${w*0.1}px ${T.neon}45, 0 0 ${w*0.18}px ${tier.glow}55, 0 8px 20px rgba(0,0,0,0.5)`,
      ...
      {/* OVR rating — FIFA-style, top-left, full size only */}
      {!compact && (
        <div style={{ position: "absolute", top: w * 0.05, left: w * 0.05, textAlign: "center" }}>
          <div style={{ fontWeight: 900, fontSize: w * 0.17, lineHeight: 1, color: dark ? T.neon : "#1A1A22", textShadow: dark ? shadow.neon : "none" }}>{ovr}</div>
          <div style={{ fontSize: Math.max(7, w * 0.032), fontWeight: 800, letterSpacing: "1.5px", color: dark ? "#EDEDF5" : "#3A3A48" }}>OVR</div>
        </div>
      )}
      {isExcited && (
        <div style={{
      ...
      {/* sport emblem — glowing crest with energy rings + motion lines */}
      <div style={{ position: "absolute", left: "50%", top: h * 0.16, transform: "translateX(-50%)", width: w * 0.5, height: w * 0.5, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ position: "absolute", width: "100%", height: "100%", borderRadius: "50%", background: `radial-gradient(circle, ${accentColor}45 0%, transparent 70%)` }} />
        <div style={{ position: "absolute", width: "100%", height: w * 0.022, background: accentColor, opacity: 0.5, transform: "rotate(20deg)", borderRadius: 4 }} />
        <div style={{ position: "absolute", width: "80%", height: w * 0.016, background: accentColor, opacity: 0.35, transform: "rotate(20deg) translateY(160%)", borderRadius: 4 }} />
        <div style={{ position: "absolute", width: "100%", height: "100%", borderRadius: "50%", border: `${Math.max(1, w * 0.01)}px solid ${accentColor}55` }} />
        <div style={{ position: "absolute", width: "78%", height: "78%", borderRadius: "50%", border: `${Math.max(1, w * 0.008)}px solid ${accentColor}80` }} />
        <div style={{
          position: "relative", width: "60%", height: "60%", borderRadius: "50%",
          background: dark ? "#0A0A0F" : "rgba(0,0,0,0.42)", display: "flex", alignItems: "center", justifyContent: "center",
          fontSize: w * 0.26, boxShadow: `0 0 ${w*0.06}px ${accentColor}70, inset 0 0 ${w*0.03}px rgba(0,0,0,0.6)`,
        }}>{sportIcon}</div>
      </div>
      {/* hero streak stat */}
      <div style={{
        ...
        <div style={{
          fontWeight: 900, fontSize: w * (compact ? 0.42 : 0.36), lineHeight: 1, color: accentColor,
      ...
      {/* FIFA-style attribute row — real stats, full size only */}
      {!compact && (
        <div style={{ position: "absolute", left: "6%", right: "6%", top: h * 0.75, display: "flex", justifyContent: "space-between" }}>
          {([["CON", consistency], ["MOM", momentum], ["REC", recovery], ["PRG", progress]] as [string, number][]).map(([label, val]) => (
            <div key={label} style={{ textAlign: "center", flex: 1 }}>
              <div style={{ fontWeight: 900, fontSize: Math.max(9, w * 0.075), color: dark ? "#F0F0F8" : "#1A1A22" }}>{val}</div>
              <div style={{ fontSize: Math.max(6, w * 0.032), fontWeight: 800, letterSpacing: "1px", color: dark ? "#AEB3C4" : "#5A5A6A" }}>{label}</div>
              <div style={{ marginTop: 2, height: Math.max(2, w * 0.012), background: "rgba(0,0,0,0.25)", borderRadius: 2, overflow: "hidden" }}>
                <div style={{ width: `${val}%`, height: "100%", background: accentColor }} />
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ padding: "20px", background: T.surface, borderRadius: 18, border: `1px solid ${T.border}`, marginBottom: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
        {([["HEIGHT (cm)", "height", "175"], ["WEIGHT (kg)", "weight", "75"]] as [string, "height" | "weight", string][]).map(([label, key, ph]) => (
          <div key={key}>
...
function CardSetup({ onSave, onBack, trialMode }: { onSave: (c: CardProfile) => void; onBack: () => void; trialMode?: boolean }) {
  ...
  return (
    <div style={{ minHeight:"100%", background:T.bg, display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ padding:"20px 20px 0", position:"sticky", top:0, background:T.bg, zIndex:10 }}>
        ...
        {trialMode && (
          <div style={{ display:"flex", alignItems:"center", gap:6, background:T.neonL, border:`1px solid ${T.neon}40`, borderRadius:10, padding:"8px 10px", marginBottom:12 }}>
            <Clock size={13} color={T.neon} />
            <span style={{ fontSize:11, fontWeight:700, color:T.textPrimary }}>Your 12-hour Elite trial starts the moment you save this card.</span>
          </div>
        )}
      </div>
      <div style={{ flex:1, overflowY:"auto", padding:"0 20px 32px" }}>
        ...
        <button onClick={() => onSave({ accentColor, nickname: nickname || "Comeback Card" })}
          ...
          {trialMode ? "Start My 12h Trial ✨" : nickname ? `Save ${nickname} ✨` : "Save My Card ✨"}
        </button>
      </div>
    </div>
  );
...
function AppDashboard({ saved, onReset, onUpgrade, onOpenAccount, authUser }: {
  saved: SavedState; onReset: () => void; onUpgrade: (code: string, card: CardProfile) => void;
  onOpenAccount: () => void; authUser: FirebaseUser | null;
}) {
  const [tab, setTabRaw] = useState<"home" | "today" | "protocol" | "squad" | "card">(() => loadLastTab());
  const setTab = useCallback((t: "home" | "today" | "protocol" | "squad" | "card") => {
  ...
  // Real Trading Card attributes — CON is the % of logged days where every
  // task got checked off, REC is supplement/protocol adherence across all
  // logged days. Both come straight from the actual history in `saved`, so
  // the card's ratings reflect real behavior rather than being decorative.
  const cardConsistencyPct = (() => {
    const entries = Object.entries(saved.completedTasks || {});
    if (!entries.length) return undefined;
    let fullDays = 0;
    for (const [dateKey, arr] of entries) {
      const dNum = Math.max(1, daysBetween(saved.startDate, dateKey) + 1);
      const expected = DAY_PROTOCOLS[dNum % DAY_PROTOCOLS.length]?.length || arr.length;
      if (expected > 0 && arr.filter(Boolean).length >= expected) fullDays++;
    }
    return Math.round((fullDays / entries.length) * 100);
  })();
  const cardRecoveryPct = (() => {
    const entries = Object.entries(saved.suppChecked || {});
    if (!entries.length) return undefined;
    let total = 0, done = 0;
    for (const [, arr] of entries) { total += arr.length; done += arr.filter(Boolean).length; }
    return total > 0 ? Math.round((done / total) * 100) : undefined;
  })();
        ...
        <div style={{ flex: 1, overflowY: "auto", background: T.bg, padding: "20px 18px 100px" }}>
          ...
          {trialActive && (
            <div style={{ marginBottom: 16 }}>
              <button onClick={() => setTab("card")} style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, background: T.neonL, border: `1px solid ${T.neon}50`, borderRadius: 12, padding: "10px 12px", cursor: "pointer", textAlign: "left" }}>
          ...
          {eliteActive && saved.card && (
            <NeonCard style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <StatsCard dayNum={dayNum} streak={saved.streak} mood={cardMood} size={72} accentColor={saved.card.accentColor} sportIcon={sport?.icon || "🏆"} consistencyPct={cardConsistencyPct} recoveryPct={cardRecoveryPct} progressPct={pct} />
                <div style={{ flex: 1 }}>
          ...
          <div style={{ padding: "0 18px" }}>
            ...
            {allDone && (
              <Card style={{ background: T.neonL, border: `1.5px solid ${T.neon}`, marginTop: 10, textAlign: "center", padding: "20px 16px", boxShadow: shadow.neon }}>
                ...
                {eliteActive && saved.card && <div style={{ marginTop:12,display:"flex",justifyContent:"center" }}><StatsCard dayNum={dayNum} streak={saved.streak} mood="excited" size={80} accentColor={saved.card.accentColor} sportIcon={sport?.icon || "🏆"} consistencyPct={cardConsistencyPct} recoveryPct={cardRecoveryPct} progressPct={pct} /></div>}
              </Card>
            )}
          </div>
        </div>
            ...
            <Card style={{ background: "#1A0808", border: `1.5px solid ${T.red}40`, marginBottom: 16 }}>
              <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: T.textSecondary, lineHeight: 1.5 }}>Your streak and protocol progress are safe — upgrade to Elite any time to get your Trading Card back.</div>
                </div>
              </div>
            </Card>
          )}
            ...
            <>
              ...
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
                <div style={{ background: T.surface, borderRadius: 24, padding: 24, border: `2px solid ${T.neon}50`, boxShadow: shadow.neon, display: "inline-flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                  <StatsCard dayNum={dayNum} streak={saved.streak} mood={cardMood} size={170} accentColor={saved.card.accentColor} sportIcon={sport?.icon || "🏆"} consistencyPct={cardConsistencyPct} recoveryPct={cardRecoveryPct} progressPct={pct} />
                  <div style={{ textAlign: "center" }}>
...
export default function App() {
  ...
  const [pendingIsTrial, setPendingIsTrial] = useState(false);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!s) {
        ...
        // injury/severity/goal/paywall/card screens), restore exactly where
        // they left off instead of bouncing them back to Welcome.
        ...
        if (progress) {
          ...
          if (progress.pendingPlan) setPendingPlan(progress.pendingPlan);
          if (progress.pendingLicense) setPendingLicense(progress.pendingLicense);
          if (progress.pendingIsTrial) setPendingIsTrial(true);
          const validScreens: Screen[] = ["welcome", "sport", "injury", "severity", "goal", "paywall", "card"];
  ...
  useEffect(() => {
    ...
    saveOnboardingProgress({
      ...
      pendingPlan, pendingLicense, pendingIsTrial,
    });
  }, [screen, sport, injury, severity, goal, pendingPlan, pendingLicense, pendingIsTrial]);
  ...
  const finalizeSetup = (plan: "pro" | "elite" | "trial", code: string, card: CardProfile | null) => {
  ...
  const handlePlanUnlock = (plan: "pro" | "elite", code: string) => {
    setPendingPlan(plan); setPendingLicense(code); setPendingIsTrial(false);
    if (plan === "elite") setScreen("card");
    else finalizeSetup(plan, code, null);
  };
  const handleTrial = () => {
    setPendingPlan("elite"); setPendingLicense(""); setPendingIsTrial(true);
    setScreen("card");
  };
  const handleCardSave = (card: CardProfile) => {
    if (!pendingPlan) return;
    finalizeSetup(pendingIsTrial ? "trial" : pendingPlan, pendingLicense || "", card);
  };
  const handleUpgradeToElite = (code: string, card: CardProfile) => {
    const s = loadState();
    if (!s) return;
    const updated: SavedState = { ...s, plan: "elite", licenseCode: code, card, trialEndsAt: null };
  ...
  const handleReset = () => {
    ...
    setPendingPlan(undefined); setPendingLicense(undefined); setPendingIsTrial(false); setScreen("welcome");
  };
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, height: "100dvh", overflow: "hidden", display: "flex", flexDirection: "column", fontFamily: "'Inter',sans-serif", background: T.bg, overscrollBehavior: "none" }}>
      ...
      {screen === "card"     && <CardSetup onSave={handleCardSave} onBack={() => setScreen("paywall")} trialMode={pendingIsTrial} />}
